...
def post_handle():
    ...
    if request.method == 'POST':
        ...
        if "tag" in input.keys():
            ...
            url = "https://api.stackexchange.com/2.2/questions?pagesize=10" + "&fromdate=" + str(
                start) + "&todate=" + str(int(
                g.request_start_time)) + "&order=desc&sort=votes&tagged=" + tag + "&site=stackoverflow&filter=" + flter_var
            ...
            url = "https://api.stackexchange.com/2.2/questions?pagesize=10" + "&fromdate=" + str(
                start) + "&todate=" + str(int(
                g.request_start_time)) + "&order=desc&sort=creation&tagged=" + tag + "&site=stackoverflow&filter=" + flter_var
...
def addFooter(body, score, time, indentation):
    body += "<p style=\"color:gray; font-size:80%;margin-left: " + indentation + "\">created at: " + str(
        time) + "<span style=\"color:green;\">   |   score: " + str(score) + "</span></p>"
    return body
