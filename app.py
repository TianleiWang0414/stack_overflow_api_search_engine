...
def post_handle():
    ...
    if request.method == 'POST':
        ...
        if "tag" in input.keys():
            ...
            conn= httplib2.Http()
            url="https://api.stackexchange.com/2.2/questions?pagesize=10&order=desc&sort=votes&tagged="+tag+"&site=stackoverflow&filter="+flter_var
            ...
            url = "https://api.stackexchange.com/2.2/questions?pagesize=10&order=desc&sort=creation&tagged=" + tag + "&site=stackoverflow&filter=" + flter_var
            f = conn.request(url)[1]
            f = f.decode('utf8')
            content = json.loads(f)
            if 'error_id' in content:
